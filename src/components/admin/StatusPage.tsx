...
import { buildComponents, LEVEL_LABEL, type Level } from "@/lib/status-model";
import { recordSample } from "@/lib/uptime-history";
...
export function StatusPage() {
  ...
  // Every poll writes one observation per component, keeping the worst level
  // seen that day. Writing notifies the store, and each strip is subscribed —
  // so there is no signal to thread down and no state to bump here.
  const components =
  ...
  useEffect(() => {
    ...
    for (const c of components) levels[c.key] = c.level;
    recordSample(levels);
    // Keyed on the snapshot timestamps so this runs once per fetched snapshot
    // rather than on every render.
  }, [ops.data?.generatedAt, health.data]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <Container size="page" className="flex flex-col gap-8 pt-8">
      ...
      {components ? (
        <section className="flex flex-col gap-3">
          <div>
            <h2 className="text-section font-bold text-text">Uptime</h2>
            <p className="mt-0.5 text-helper text-text-faint">
              Recorded by this browser while the page is open — grey days are
              ones nobody was watching, not healthy ones.
            </p>
          </div>
            ...
            {components.map((c, i) => (
              ...
              >
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <span className={cx("text-label font-medium", LEVEL_TEXT[c.level])}>
                    {LEVEL_LABEL[c.level]}
                  </span>
                </div>
                <UptimeStrip componentKey={c.key} />
              </div>
            ))}
          </div>
          <p className="text-helper text-text-faint">
            A real 90-day bar comes from an external prober sampling the API
            from outside, or from the backend recording a sample on a schedule.
            Until one exists, only the days this page was open can be shown —
            which is why most of the strip is grey rather than green.
          </p>
        </section>
      ) : null}
